(function () {
    ...
    let parser = {
        "parser_code" : [
            ...
            ["ДК" , "Кросс дебетовые карты: "],
            ["КК" , "Кросс кредитные карты: "],
            ["СДК" , "Селфи дебетовые карты: "],
            ["СКК" , "Селфи кредитные карты: "],
            ["ДКх5_5" , "Кросс дебетовые карты х5 пятёрочка"],
            ["ККх5_5" , "Кросс кредитные карты х5 пятёрочка"],
            ["ДКх5_4" , "Кросс дебетовые карты х5 перекресток"],
            ["ККх5_4" , "Кросс кредитные карты х5 перекресток"]
        ],
        "parser_settings" : {
            "CR_DK_C": {
                "code" : "КДК",
    ...
    module.exports = this.EventHandlerClass = new class{
        ...
        onHelpRequired = (type) => {
            if (type === "adm")
                return cfg.help_info.admin
            else
                return cfg.help_info.client
        }
    }
    function generateId() {
